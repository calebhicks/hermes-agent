...
from agent.prompt_builder import (
    ...
    HERMES_AGENT_HELP_GUIDANCE, HERMES_AGENT_HELP_GUIDANCE_NO_SKILLS, kanban_worker_guidance,
...
def _tool_guidance_block(agent: Any) -> Optional[str]:
    ...
    # the helper fallback covers code paths that bypass agent_init.
    _kanban_guidance = getattr(agent, "_kanban_worker_guidance", None)
    if _kanban_guidance is None:
        _kanban_guidance = kanban_worker_guidance(names)
    tool_guidance = [
        memory_guidance,
...
def _skills_prompt(agent: Any) -> str:
    ...
    return _pb.build_skills_system_prompt(available_tools=agent.valid_tool_names, available_toolsets=avail_toolsets,
                                         ...
                                         index_mode=getattr(agent, "_skill_index_mode", "full"),
