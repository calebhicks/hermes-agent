...
def _load_tools(agent, enabled_toolsets, disabled_toolsets):
    ...
    # Kanban worker lifecycle guidance is session-static. Activation requires
    # BOTH the dispatcher's task binding (HERMES_KANBAN_TASK) and the worker
    # tool: orchestrator profiles can carry kanban_show in ordinary interactive
    # sessions, so tool presence alone would misfire the worker protocol.
