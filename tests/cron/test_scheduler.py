...
class TestDeliverResultTimeoutCancelsFuture:
    ...
    STARTED — tracked by a threading.Event set at coroutine entry, NOT by
    future.cancel()'s return value.  cancel() returning True does not prove
    the coroutine never ran: an asyncio task suspended at an await between
    per-bubble sends cancels successfully after part of the payload is already
    on the wire (2026-08-10 duplicate-brief incident).  A started delivery is
    never cancelled and never re-sent (assume-delivered); only a provably
    never-started coroutine is cancelled and retried via standalone, or the
    message is silently dropped.  Regression for #38922 and the 2026-08-10
    follow-up.
    """
    ...
    def test_started_delivery_is_never_cancelled_or_resent(self):
        """2026-08-10 regression: a delivery mid-payload (coroutine entered,
        suspended between bubbles) times out AND is still cancellable —
        future.cancel() would return True.  The scheduler must neither cancel
        it (truncates the remaining bubbles) nor run the standalone fallback
        (re-sends the whole payload as duplicates).  The started flag, not
        cancel()'s return value, decides."""
        from gateway.config import Platform
        from concurrent.futures import Future
        ...
        def suspended_cancel():
            # Simulates an asyncio task suspended at an await between bubbles:
            # cancellation SUCCEEDS even though bubbles already went out.
            cancel_calls.append(True)
            return True
        ...
        def fake_run_coro(coro, _loop):
            # Advance the wrapped coroutine one step so its entry runs and the
            # delivery-started Event is set — the "bubbles already on the
            # wire" state.  It may also run to completion (StopIteration) if
            # nothing genuinely suspends; either way it has STARTED.
            try:
                coro.send(None)
        ...
        # 1. The started delivery was never cancelled.
        assert cancel_calls == [], "a started delivery must never be cancelled"
        # 2. Delivery is reported successful (assume-delivered).
        assert result is None, f"expected assume-delivered, got error: {result!r}"
        # 3. No standalone resend — that is exactly the duplicate-brief bug.
        standalone_send.assert_not_awaited()
