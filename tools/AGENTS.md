...
subagent_auto_approve, inherit_mcp_toolsets, blocked_child_toolsets, max_iterations`. The operator-owned
`blocked_child_toolsets` denylist is applied after inheritance and role expansion and is also passed through
`disabled_toolsets`, so a named toolset cannot return through a composite or registry refresh. **Child processes:** a child's background
